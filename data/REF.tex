...
\title{\includegraphics[width=0.30\textwidth]{pict/archoad.png}\hfill\includegraphics[width=0.30\textwidth]{pict/customer.png}\\\bigskip\bigskip
...
\abstract{Ce rapport décrit le résultat de l'évaluation réalisée à \textsl{Etablissement de référence} en 2020. L'évaluation initiale a été contrôlée le \today{} par Bruce Banner. Cette évaluation repose sur un questionnaire établit conformément aux règles d'hygiène de l'ANSSI.}
...
\begin{center}
\begin{tikzpicture}
\tikzstyle{every node} = [draw=myRed, rounded corners=4pt, fill=myRed!20, text width=300pt, inner sep=5pt]
...
\section{Analyse de l'évaluation pour l'année 2020}
...
\tikzstyle{myLabel} = [font=\scriptsize, text width=45pt, text badly centered, inner sep=0pt]
\tikzstyle{myLegend} = [font=\scriptsize, anchor=west]
\tikzstyle{myTitle} = [font=\bfseries, fill=myBeige, text width=90pt, text badly centered, inner sep=5pt, rounded corners=2pt]
\tikzstyle{myMatrix} = [fill=myBeige, inner sep=5pt, rounded corners=2pt]
\begin{center}
...
\path (1*\axisAngle:\labelDist) node [myLabel] {Sensibilisation};
\path (2*\axisAngle:\labelDist) node [myLabel] {Connaître le SI};
\path (3*\axisAngle:\labelDist) node [myLabel] {Authentifier};
\path (4*\axisAngle:\labelDist) node [myLabel] {Sécuriser les postes};
\path (5*\axisAngle:\labelDist) node [myLabel] {Sécuriser le réseau};
\path (6*\axisAngle:\labelDist) node [myLabel] {Sécuriser l'administration};
\path (7*\axisAngle:\labelDist) node [myLabel] {Nomadisme};
\path (8*\axisAngle:\labelDist) node [myLabel] {Update du SI};
\path (9*\axisAngle:\labelDist) node [myLabel] {Superviser};
\path (10*\axisAngle:\labelDist) node [myLabel] {Risques};

\draw [myShape, color=year1] (D1-4) -- (D2-4) -- (D3-5) -- (D4-6) -- (D5-6) -- (D6-3) -- (D7-6) -- (D8-3) -- (D9-7) -- (D10-6) -- cycle;

\fill [myShapeFill, color=year1!20] (D1-4) -- (D2-4) -- (D3-5) -- (D4-6) -- (D5-6) -- (D6-3) -- (D7-6) -- (D8-3) -- (D9-7) -- (D10-6) -- cycle;

\draw [myShape, color=objectiv] (D1-4) -- (D2-7) -- (D3-3) -- (D4-4) -- (D5-5) -- (D6-6) -- (D7-7) -- (D8-6) -- (D9-5) -- (D10-4) -- cycle;

\matrix [myMatrix, xshift=180pt, yshift=-100pt] {
...
\draw [myShape, color=year3] (D1-2) -- (D2-6) -- (D3-2) -- (D4-6) -- (D5-5) -- (D6-7) -- (D7-2) -- (D8-3) -- (D9-5) -- (D10-7) -- cycle;

\fill [myShapeFill, color=year3!20] (D1-2) -- (D2-6) -- (D3-2) -- (D4-6) -- (D5-5) -- (D6-7) -- (D7-2) -- (D8-3) -- (D9-5) -- (D10-7) -- cycle;

\draw [myShape, color=year2] (D1-5) -- (D2-6) -- (D3-4) -- (D4-6) -- (D5-5) -- (D6-6) -- (D7-4) -- (D8-5) -- (D9-6) -- (D10-7) -- cycle;

\fill [myShapeFill, color=year2!20] (D1-5) -- (D2-6) -- (D3-4) -- (D4-6) -- (D5-5) -- (D6-6) -- (D7-4) -- (D8-5) -- (D9-6) -- (D10-7) -- cycle;
...
\textbf{Question n\textdegree 1.3.5} La liste d'exigence détaille la mise en oeuvre du maintien à niveau de la sécurité dans le temps.
...
\textbf{Question n\textdegree 3.6.1} L'entité met en oeuvre une authentification forte pour ses systèmes sensibles.
...
\textbf{Question n\textdegree 3.6.2} L'entité met en oeuvre une authentification forte pour ses comptes d'administration.
...
\textbf{Question n\textdegree 3.6.3} L'entité met en oeuvre une authentification forte pour l'ensemble de ses utilisateurs.
...
\subsection{Sécuriser les postes de travail}
...
\textbf{Question n\textdegree 4.2.3} Le rechargement électrique des smartphones et tablettes est interdit sur les postes utilisateurs.
...
\textbf{Question n\textdegree 4.3.1} L'entité met en oeuvre un outil de gestion centralisée de type Active Directory.
...
\textbf{Question n\textdegree 5.2.1} Le réseau Wi-Fi met en oeuvre un chiffrement des flux robuste (mode WPA2 avec AES CCMP).
...
\textbf{Question n\textdegree 5.7.2} Les réseaux privés virtuels (VPN) mis en oeuvre utilisent le standard IPSEC.
...
\textbf{Question n\textdegree 6.2.2} L'entité met en oeuvre un cloisonnement logique ou physique de son réseau d'administration.
...
\textbf{Question n\textdegree 6.3.2} L'entité met en oeuvre un magasin d'applications validées du point de vue de la sécurité.
...
\textbf{Question n\textdegree 7.3.1} L'entité met en oeuvre un réseau privé virtuel (VPN) entre son système d'information et les terminaux nomades.
...
\textbf{Question n\textdegree 7.3.6} L'entité met en oeuvre un mécanisme d'authentification forte pour les ordinateurs nomades. La démarche est formalisée dans une procédure interne à l'entité.
...
\textbf{Question n\textdegree 7.4.1} L'entité met en oeuvre une solution de gestion centralisée de sa flotte de terminaux nomades.
...
\textbf{Question n\textdegree 9.4.1} L'entité a formellement désigné un référent en sécurité des systèmes d'information.
...
\textbf{Question n\textdegree 10.1.1} L'entité met en oeuvre une démarche d'analyse des risques informationnels (EBIOS).
