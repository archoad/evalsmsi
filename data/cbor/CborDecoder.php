...
class CborDecoder {
	...
	public static function decode($bufOrBin) {
		...
		if ($offset !== $buf->getLength()) {
			throw new Exception('Unused bytes after data item.');
		}
		return $result;
	}
	...
	protected static function _parseFloatSimple($val, ByteBuffer $buf, &$offset) {
		switch ($val) {
			...
			case 29:
			case 30:
				throw new Exception('Reserved value used.');

			case 31:
				throw new Exception('Indefinite length is not supported.');
		}
	...
	protected static function _parseSimple($val) {
		...
		throw new Exception(sprintf('Unsupported simple value %d.', $val));
	}
	...
	protected static function _parseItemData($type, $val, ByteBuffer $buf, &$offset) {
		...
		throw new Exception(sprintf('Unknown major type %d.', $type));
	}
	...
	protected static function _parseMap(ByteBuffer $buf, &$offset, $count) {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if (!\is_int($mapKey) && !\is_string($mapKey)) {
				throw new Exception('Can only use strings or integers as map keys');
			}
